...
@app.route('/')
def home():
    return "🤖 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 🦁 is Running!"
...
# --- Configuration ---
TOKEN = '8930421045:AAHVKc8JMKiQUQQXTuTs94bUhJ_uROwbr_UU'
OWNER_ID = 6678624575
ADMIN_ID = 6678624575
YOUR_USERNAME = '@Nejad1234'
UPDATE_CHANNEL = 'https://t.me/Nejad244'
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🚀 <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: STARTING SCRIPT</b> 🚀 ║
╠══════════════════════════════════════╣
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: SCRIPT RUNNING</b> ✅   ║
╠══════════════════════════════════════╣
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║     ❌ <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: SCRIPT FAILED</b> ❌     ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🟢 <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: STARTING NODE.JS</b> 🟢║
╠══════════════════════════════════════╣
║ 📄 File: <code>{file_name[:25]}</code>
║ 👤 User: {script_owner_id}
║ 🔄 Attempt: {attempt}/{max_attempts}
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: NODE.JS RUNNING</b> ✅  ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ 🆔 <b>PID:</b> {process.pid}
║ ⏱️ <b>Started:</b> {datetime.now().strftime('%H:%M:%S')}
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║     ❌ <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: NODE.JS FAILED</b> ❌    ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ ❗ <b>Exit Code:</b> {process.returncode}
╠══════════════════════════════════════╣
...
def stats_command(message):
    ...
    msg = send_spinner_animation(message.chat.id, "𝐆𝐚𝐭𝐡𝐞𝐫𝐢𝐧𝐠 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐬𝐭𝐚𝐭𝐬...", duration=2)
...
def speed_command(message):
    ...
    msg = send_spinner_animation(message.chat.id, "𝐓𝐞𝐬𝐭𝐢𝐧𝐠 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐬𝐩𝐞𝐞𝐝...", duration=2)
...
def running_command(message):
    """Show running bots"""
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐅𝐞𝐭𝐜𝐡𝐢𝐧𝐠 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐛𝐨𝐭𝐬...", duration=1)
    running_bots = []
...
def process_broadcast(message):
    ...
    if not broadcast_text:
        ...
        return
    progress_msg = bot.send_message(message.chat.id, "📢 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭...")
    success = 0
    failed = 0
...
"""
    ...
    result_text = f"""
╔══════════════════════════════════════╗
║     ✅ <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 𝐂𝐎𝐌𝐏𝐋𝐄Τ𝗘</b> ✅ ║
╠══════════════════════════════════════╣
║
║  📤 𝐓𝐨𝐭𝐚𝐥: {total}
║  ✅ 𝐒𝐮𝐜𝐜𝐞𝐬𝐬: {success}
...
"""
    ...
    try:
        bot.send_message(target_user, f"🎉 𝐘𝐨𝐮'𝐯𝐞 𝐛𝐞𝐞𝐧 𝐬𝐮𝐛𝐬𝐜𝐫𝐢𝐛𝐞𝐝 𝐟𝐨𝐫 {days} 𝐝𝐚𝐲𝐬 𝐛𝐲 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶!")
    except:
        pass
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel":
        bot.send_message(message.chat.id, f"📢 𝐉𝐨𝐢𝐧 𝐨𝐮𝐫 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐮𝐩𝐝𝐚𝐭𝐞𝐬:\n{UPDATE_CHANNEL}")
...
def show_user_files(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐋𝐨𝐚𝐝𝐢𝐧𝐠 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐟𝐢𝐥𝐞𝐬...", duration=1)
    files = user_files.get(user_id, [])
    if not files:
        text = f"""
╔══════════════════════════════════════╗
║       📂 <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: YOUR FILES</b> 📂   ║
╠══════════════════════════════════════╣
║
...
def show_subscriptions(message):
    ...
    text = f"""
╔══════════════════════════════════════╗
║     💳 <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: SUBSCRIPTIONS</b> 💳    ║
╠══════════════════════════════════════╣
║
...
def show_file_actions(call, file_name):
    ...
    for name, ftype in user_files.get(user_id, []):
        if name == file_name:
            ...
            break
    type_icon = "🐍" if file_type == "py" else "🟨" if file_type == "js" else "📄"
    status = "🟢 𝐑uu𝐧𝐧𝐢𝐧𝐠" if is_running else "🔴 𝐒𝐭𝐨𝐩𝐩𝐞𝐝"
    text = f"""
╔══════════════════════════════════════╗
║       📄 <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: FILE</b> 📄         ║
╠══════════════════════════════════════╣
║
...
def stop_all_bots(call):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        ...
        return
    bot.answer_callback_query(call.id, "🛑 𝐒𝐭𝐨𝐩𝐩𝐢𝐧𝐠 𝐚𝐥𝐥 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐛𝐨𝐭𝐬...")
    stopped = 0
...
def show_admin_logs(call):
    ...
    try:
        ...
        conn.close()
        if logs:
            text = "📋 <b>𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶: RECENT LOGS</b>\n"
            for log in logs:
                text += f"👤 {log[0]} | {log[1]}\n{log[2][:30]}...\n🕐 {log[3][:16]}\n"
        else:
            text = "📋 𝐍𝐨 𝐥𝐨𝐠𝐬."
...
def cleanup_on_exit():
    ...
    logger.info("𝐂𝐥𝐞𝐚𝐧𝐢𝐧𝐠 𝐮𝐩 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶...")
    for script_key in list(bot_scripts.keys()):
        try:
            script_info = bot_scripts[script_key]
            kill_process_tree(script_info)
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 🦁 𝐁𝐨𝐭...")
    ...
    while True:
        try:
            ...
            logger.error(f"𝗠𝗱 𝗡𝗲𝗷𝗮𝗱 𝗔𝗹𝗶 𝐞𝐫𝐫𝐨𝐫: {e}", exc_info=True)
            time.sleep(5)
